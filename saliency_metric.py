...
class cal_em(object):
    ...
    def show(self):
        return np.mean(self.prediction)
###########################################
class cal_f2m(object):
    def __init__(self, num, thds=255):
        self.num = num
        self.thds = thds
        self.precision = np.zeros((self.num, self.thds))
        self.recall = np.zeros((self.num, self.thds))
        self.changeable_fms = np.zeros((self.num, self.thds))
        self.idx = 0

    def update(self, pred, gt):
        if gt.max() != 0:
            precision, recall, changeable_fms = self.cal(pred, gt)
            self.precision[self.idx, :] = precision
            self.recall[self.idx, :] = recall
            self.changeable_fms[self.idx, :] = changeable_fms
        self.idx += 1
    ...
    def cal(self, pred, gt):
        ...
        targetHist = np.cumsum(np.flip(targetHist))
        nontargetHist = np.cumsum(np.flip(nontargetHist))

        precision = targetHist / (targetHist + nontargetHist + 1e-8)
        recall = targetHist / (np.sum(gt) + 1e-8)

        numerator = (1 + 0.3) * precision * recall
        denominator = 0.3 * precision + recall + 1e-8
        changeable_fms = numerator / denominator
        return precision, recall, changeable_fms

    def show(self):
        assert self.num == self.idx
        precision = self.precision.mean(axis=0)
        recall = self.recall.mean(axis=0)
        changeable_fm = self.changeable_fms.mean(axis=0)
        max_fm = changeable_fm.max()
        return max_fm, changeable_fm, precision, recall


# 新增S2-measure
class cal_s2m(object):
    def __init__(self, alpha=0.5):
        self.scores = []
        self.alpha = alpha
    ...
    def update(self, pred, gt):
        gt = gt > 0.5
        if np.all(gt) or not np.any(gt):
            self.scores.append(0 if not np.any(gt) else 1)
            return
    ...
    def cal(self, pred, gt):
        def ssim(pred, gt):
            ...
            return alpha / (beta + 1e-8) if alpha != 0 else 0

        def object_score(pred, gt):
            fg = pred[gt]
            bg = (1 - pred)[~gt]
            u = np.mean(gt)
            return u * (2 * np.mean(fg) / (np.mean(fg) ** 2 + 1 + np.std(fg))) + \
                (1 - u) * (2 * np.mean(bg) / (np.mean(bg) ** 2 + 1 + np.std(bg)))
        ...
        def region_score(pred, gt):
            ...
            weights = [w / (h * w) for w in weights]

            score = 0
            for (y1, x1, y2, x2), w in zip(regions, weights):
                score += w * ssim(pred[y1:y2, x1:x2], gt[y1:y2, x1:x2])
            return score

        return self.alpha * object_score(pred, gt) + (1 - self.alpha) * region_score(pred, gt)
...
class cal_e2m(object):
    ...
    def update(self, pred, gt):
        gt = gt > 0.5
        if np.all(gt) or not np.any(gt):
            self.scores.append(1 if np.all(gt) else 0)
            return
    ...
    def cal(self, pred, gt):
        def align_measure(pred, gt):
            mu_pred = np.mean(pred)
            mu_gt = np.mean(gt)
            align_pred = pred - mu_pred
            align_gt = gt - mu_gt
            return 2 * align_pred * align_gt / (align_pred ** 2 + align_gt ** 2 + 1e-8)

        align = align_measure(pred, gt)
        enhanced = np.square(align + 1) / 4
